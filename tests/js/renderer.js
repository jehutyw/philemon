...
function run(check) {
    check("the launcher's own Vulkan choice retries once on OpenGL",
          command("vulkan", "1", "/usr/bin/philemon"),
          "/usr/bin/env QSG_RHI_BACKEND=opengl /usr/bin/philemon --gui")
    // src/gui.rs removes PHILEMON_RENDERER_AUTOMATIC on an explicit choice, so Quickshell.env answers null.
    check("an operator's own Vulkan choice is never retried",
          command("vulkan", null, "/usr/bin/philemon"), "none")
    check("a marker beside another renderer retries nothing",
          command("opengl", "1", "/usr/bin/philemon"), "none")
    check("and neither does the renderer a retry already moved to",
          command("opengl", null, "/usr/bin/philemon"), "none")
    ...
    check("no other marker value is the launcher's",
          command("vulkan", "0", "/usr/bin/philemon"), "none")
    check("an empty PHILEMON_BIN falls back to the name on PATH",
          command("vulkan", "1", ""),
          "/usr/bin/env QSG_RHI_BACKEND=opengl philemon --gui")

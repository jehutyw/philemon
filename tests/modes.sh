...
unset QSG_RHI_BACKEND PHILEMON_RENDERER_AUTOMATIC VK_DRIVER_FILES VK_ICD_FILENAMES
fail=0
...
D="$FIXTURE_ROOT/philemon-pick-test-$$"
sandbox_make "$D"
out=$(env -u PHILEMON_PICKER $BIN --pick "$D/reply.json" 2>&1 </dev/null); rc=$?
check "--pick with no request refuses" "2" "$rc"
check "--pick with no request names PHILEMON_PICKER" "1" "$(echo "$out" | grep -c 'needs PHILEMON_PICKER')"
out=$(env PHILEMON_PICKER= $BIN --pick "$D/reply.json" 2>&1 </dev/null); rc=$?
check "--pick with an empty request refuses" "2" "$rc"
check "--pick with an empty request gives the same sentence" "1" "$(echo "$out" | grep -c 'needs PHILEMON_PICKER')"
...
out=$(env WAYLAND_DISPLAY= DISPLAY= PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
check "an empty display refuses" "1" "$(echo "$out" | grep -c 'no graphical session')"
out=$(env -u DISPLAY WAYLAND_DISPLAY= PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
check "an empty WAYLAND_DISPLAY alone refuses" "1" "$(echo "$out" | grep -c 'no graphical session')"
out=$(env -u WAYLAND_DISPLAY DISPLAY= PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
...
out=$(env -u DISPLAY WAYLAND_DISPLAY=philemon-modes-test-display PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
check "a non-empty WAYLAND_DISPLAY is accepted" "1" "$(echo "$out" | grep -c 'could not start the shell')"
out=$(env -u WAYLAND_DISPLAY DISPLAY=:99 PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
check "a non-empty DISPLAY is accepted" "1" "$(echo "$out" | grep -c 'could not start the shell')"
out=$(env WAYLAND_DISPLAY= DISPLAY=:99 PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
check "an empty WAYLAND_DISPLAY does not mask DISPLAY" "1" "$(echo "$out" | grep -c 'could not start the shell')"
out=$(env WAYLAND_DISPLAY=philemon-modes-test-display DISPLAY= PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
check "an empty DISPLAY does not mask WAYLAND_DISPLAY" "1" "$(echo "$out" | grep -c 'could not start the shell')"
out=$(env WAYLAND_DISPLAY=philemon-modes-test-display DISPLAY=:99 PATH=/nonexistent-philemon-test-path $BIN --gui 2>&1 </dev/null)
...
# A shell PTY proves bare philemon still means the product that exists, not the reserved terminal interface.
...
printf 'PHILEMON_BIN %s\n' "$PHILEMON_BIN"
printf 'RENDERER %s\n' "$QSG_RHI_BACKEND"
printf 'AUTOMATIC %s\n' "${PHILEMON_RENDERER_AUTOMATIC-unset}"
printf 'ARGV %s\n' "$*"
printf 'PHILEMON_PATH %s\n' "${PHILEMON_PATH-unset}"
printf 'PHILEMON_SELECT %s\n' "${PHILEMON_SELECT-unset}"
STUB
chmod +x "$D/qs"
out=$(env PHILEMON_BIN=stale WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
...
check "the launched shell uses this Philemon binary" "PHILEMON_BIN $BIN_REAL" \
  "$(echo "$out" | grep '^PHILEMON_BIN ')"
...
out=$(env QSG_RHI_BACKEND=opengl PHILEMON_RENDERER_AUTOMATIC=stale WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
...
out=$(env QSG_RHI_BACKEND= WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
...
out=$(env VK_DRIVER_FILES=/nonexistent-philemon-icd VK_ICD_FILENAMES=/nonexistent-philemon-icd \
  WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
...
check "and the operator is told which call refused" "1" \
  "$(echo "$out" | grep -c 'philemon: Vulkan is unusable, vkCreateInstance answered ')"
...
out=$(env VK_DRIVER_FILES=/nonexistent-philemon-icd VK_ICD_FILENAMES=/nonexistent-philemon-icd \
  QSG_RHI_BACKEND=vulkan WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
...
good=$(env WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" \
  $BIN --gui --select /etc/hostname 2>&1 </dev/null)
broken=$(env VK_DRIVER_FILES=/nonexistent-philemon-icd VK_ICD_FILENAMES=/nonexistent-philemon-icd \
  WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" \
  $BIN --gui --select /etc/hostname 2>&1 </dev/null)
check "the working arm opens the selected file's directory" "PHILEMON_PATH /etc" "$(echo "$good" | grep '^PHILEMON_PATH ')"
check "the working arm selects the file itself" "PHILEMON_SELECT /etc/hostname" "$(echo "$good" | grep '^PHILEMON_SELECT ')"
check "the fallback arm opens the same directory" "$(echo "$good" | grep '^PHILEMON_PATH ')" "$(echo "$broken" | grep '^PHILEMON_PATH ')"
check "the fallback arm selects the same file" "$(echo "$good" | grep '^PHILEMON_SELECT ')" "$(echo "$broken" | grep '^PHILEMON_SELECT ')"
...
# A launch with no PHILEMON_BIN in the environment is the ordinary one, and it must still name this binary.
out=$(env -u PHILEMON_BIN WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
check "an unset PHILEMON_BIN is derived from the running binary" "PHILEMON_BIN $BIN_REAL" \
  "$(echo "$out" | grep '^PHILEMON_BIN ')"
...
last_arg="$D/last-arg"
# Sample input: gio open /home/philemon-sandbox/philemon-open-test-123/file.txt
...
chmod +x "$D/bin/qs"
: > "$opened"
out=$(env WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D/bin:/usr/bin:/bin" $BIN --gui 2>&1 </dev/null)
...
D="$FIXTURE_ROOT/philemon-terminal-test-$$"
sandbox_make "$D"
mkdir -p "$D/dir" "$D/bin"
printf 'hello' > "$D/file.txt"
ln -s "$D/dir" "$D/linkdir"
...
ran="$D/ran.log"
# Sample input: xdg-terminal-exec --dir=/home/philemon-sandbox/philemon-terminal-test-123/dir
# No strip-to-paren here: cut reads its OWN stat, comm is bare "cut", and its pgid is the stub's by fork.
{
  printf '#!/bin/sh\n'
  printf 'printf "FD1 %%s\\n" "$(readlink /proc/$$/fd/1)" >> %q\n' "$ran"
...
: > "$ran"
# Quickshell hands philemon --terminal a pipe and closes it, so a pipe is what the terminal must not inherit.
...
# A pipe here dies with the philemon that made it, and the terminal dies with it on its first write.
...
out=$(env PATH=/nonexistent-philemon-test-path $BIN --terminal "$D/dir" 2>&1)
rc=$?
...
chmod +x "$D/bin/qs"
: > "$ran"
env WAYLAND_DISPLAY=philemon-modes-test-display PATH="$D/bin:/usr/bin:/bin" $BIN --gui >/dev/null 2>&1 </dev/null
...
# at all, and which programs need one is the desktop database's judgement, never Philemon's. This drives
...
T="$FIXTURE_ROOT/philemon-terminal-entry-$$"
sandbox_make "$T"
mkdir -p "$T/data/applications" "$T/config" "$T/bin"
terminal_log="$T/ran.log"
{ printf '#!/bin/sh\n'; printf 'printf "HANDLER %%s\\n" "$*" >> %q\n' "$terminal_log"; } > "$T/bin/philemon-t41-handler"
chmod +x "$T/bin/philemon-t41-handler"
...
chmod +x "$T/bin/xdg-terminal-exec"
{
  ...
  printf 'Name=Philemon issue 41 handler\n'
  printf 'Exec=philemon-t41-handler %%f\n'
  printf 'Terminal=true\n'
  printf 'NoDisplay=true\n'
  printf 'MimeType=text/plain;\n'
} > "$T/data/applications/philemon-t41.desktop"
printf '[Default Applications]\ntext/plain=philemon-t41.desktop\n' > "$T/config/mimeapps.list"
printf 'hello\n' > "$T/note.txt"

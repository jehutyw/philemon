...
if command -v magick >/dev/null; then
  ...
  # The positive control on this key and this cache root; the corrupt-file case below pins the fail/philemon path the negative check names.
  check "and the same key names the thumbnail it published" "1" "$([ -e "$CACHE/large/$icc_key.png" ] && echo 1 || echo 0)"
  check "and records no failure marker against it" "0" "$([ -e "$CACHE/fail/philemon/$icc_key.png" ] && echo 1 || echo 0)"
else

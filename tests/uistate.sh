...
# Sample input: the pretty document philemon --ui-state prints, whose arrays open on `  "columns": [`
...
out=$(philemon_ui '{"view":"grid","places":{"sidebarWidth":240}}' 2>&1); rc=$?
...
check "the state directory is owner only" "700" "$(stat -c '%a' "$STATE/philemon")"
# ls -A: ui.json and its lock, and no temp file left behind by the rename.
check "no temp file survives the write" "ui.json ui.json.lock" "$(ls -A "$STATE/philemon" | sort | tr '\n' ' ' | sed 's/ $//')"

# A second patch merges rather than replacing, which is what "caller-key merge" has to mean.
philemon_ui '{"hidden":true}' >/dev/null 2>&1
check "the earlier key survives the later patch" "1" "$(grep -c '"view": "grid"' "$UI")"
check "the later key landed" "1" "$(grep -c '"hidden": true' "$UI")"

# A key from a newer Philemon is kept and rewritten as it was read, so an older Philemon cannot eat it.
fresh
mkdir -p "$STATE/philemon"
printf '{"fromANewerPhilemon":{"a":[1,"two"]},"view":"columns"}\n' > "$UI"
philemon_ui '{"hidden":true}' >/dev/null 2>&1
check "an unknown key survives a write" "1" "$(grep -c '"fromANewerPhilemon"' "$UI")"
...
fresh
out=$(philemon_ui '{"view":"miller"}' 2>&1); rc=$?
...
check "a refused patch leaves the directory and the lock it took" "ui.json.lock" "$(ls -A "$STATE/philemon" | sort | tr '\n' ' ' | sed 's/ $//')"
out=$(philemon_ui '{"notAKey":1}' 2>&1); rc=$?
check "an unknown patch key exits 2" "2" "$rc"
check "an unknown patch key names itself" "1" "$(echo "$out" | grep -c 'notAKey')"
out=$(philemon_ui 'not json at all' 2>&1); rc=$?
...
# the merged document on stdout at 0, one `philemon: ` sentence on stderr at 2, and nothing on the other
# stream either way. Nothing pinned the split, so a refusal printed to stdout would have passed.
fresh
merged=$(philemon_ui '{"view":"grid"}' 2>/dev/null); rc=$?
check "an accepted patch exits 0" "0" "$rc"
check "an accepted patch prints the document on stdout" "1" "$(echo "$merged" | grep -c '"view": "grid"')"
check "an accepted patch prints nothing on stderr" "" "$(philemon_ui '{"view":"grid"}' 2>&1 >/dev/null)"
check "a refused patch prints its sentence on stderr" "1" "$(philemon_ui '{"view":"miller"}' 2>&1 >/dev/null | grep -c '^philemon: ')"
check "a refused patch prints nothing on stdout" "" "$(philemon_ui '{"view":"miller"}' 2>/dev/null)"
check "a patch that is not JSON prints its sentence on stderr" "1" "$(philemon_ui 'not json at all' 2>&1 >/dev/null | grep -c '^philemon: ')"
check "a patch that is not JSON prints nothing on stdout" "" "$(philemon_ui 'not json at all' 2>/dev/null)"
check "two arguments prints its sentence on stderr" "1" "$(philemon_ui a b 2>&1 >/dev/null | grep -c '^philemon: ')"
check "two arguments prints nothing on stdout" "" "$(philemon_ui a b 2>/dev/null)"
...
for bad_columns in '{"columns":[]}' '{"columns":["size","date"]}' '{"columns":["name","size","size"]}'; do
  fresh
  out=$(philemon_ui "$bad_columns" 2>&1); rc=$?
  ...
  check "and leaves only the lock it took: $bad_columns" "ui.json.lock" "$(ls -A "$STATE/philemon" | sort | tr '\n' ' ' | sed 's/ $//')"
done
...
out=$(philemon_ui '{"hidden":true}' 2>&1); rc=$?
...
out=$(env WAYLAND_DISPLAY=philemon-uistate-test-display PATH=/nonexistent-philemon-test-path \
...
fresh
mkdir -p "$CONFIG/philemon"
printf '{"hiddenCols":["kind"]}\n' > "$CONFIG/philemon/view.json"
chmod 500 "$STATE"
out=$(env WAYLAND_DISPLAY=philemon-uistate-test-display PATH=/nonexistent-philemon-test-path \
      XDG_STATE_HOME="$STATE" XDG_CONFIG_HOME="$CONFIG" $BIN --gui </dev/null 2>&1)
...
check "and prints its sentence on stderr" "1" "$(philemon_ui '{"hidden":true}' 2>&1 >/dev/null | grep -c '^philemon: ')"
check "and prints nothing on stdout" "" "$(philemon_ui '{"hidden":true}' 2>/dev/null)"
...
env WAYLAND_DISPLAY=philemon-uistate-test-display PATH=/nonexistent-philemon-test-path \
...
# first: a value this Philemon refuses must never be what the first paint draws, and the two front ends
# must answer the same question the same way.
fresh
mkdir -p "$STATE/philemon"
printf '{"columns":["name","size","owner"],"density":"compact","fromANewerPhilemon":{"a":1}}\n' > "$UI"
env WAYLAND_DISPLAY=philemon-uistate-test-display PATH=/nonexistent-philemon-test-path \
    XDG_STATE_HOME="$STATE" XDG_CONFIG_HOME="$CONFIG" $BIN --gui </dev/null >/dev/null 2>&1
...
check "the settle keeps a newer Philemon's own key" "1" "$(grep -c 'fromANewerPhilemon' "$UI")"
settled=$(cat "$UI")
...
check "and the read still answers the full default shape" "1" "$(philemon_ui 2>&1 | tr -d ' \n' | grep -c '"columns":\["name","size","date"\]')"
...
check "and that file reads as the full default shape" "1" "$(philemon_ui 2>&1 | tr -d ' \n' | grep -c '"density":"normal"')"
...
fresh
mkdir -p "$STATE/philemon" "$CONFIG/philemon"
...
check "and view.json is not read in its place" "1" "$(philemon_ui 2>&1 | tr -d ' \n' | grep -c '"columns":\["name","size","date"\]')"
...
# The way in that needs no hex editor: a mode this process cannot read, which one `sudo philemon` also
...
check "and the refusal left only the lock it took" "ui.json ui.json.lock" "$(ls -A "$STATE/philemon" | sort | tr '\n' ' ' | sed 's/ $//')"
chmod 600 "$UI"
...
fresh
philemon_ui '{"hidden":false}' >/dev/null 2>&1
...
for patch in $racers; do
  philemon_ui "$patch" >/dev/null 2>&1 &
done
wait
landed=0
...
check "the racing writers left no temp behind" "ui.json ui.json.lock" "$(ls -A "$STATE/philemon" | sort | tr '\n' ' ' | sed 's/ $//')"

# A write killed part way through leaves the previous file byte for byte, because the rename is last.
fresh
philemon_ui '{"view":"list"}' >/dev/null 2>&1
before=$(cat "$UI")
philemon_ui '{"view":"grid"}' >/dev/null 2>&1
after=$(cat "$UI")
...
temps=$(ls -A "$STATE/philemon" | grep -c '^ui\.json\.[0-9]\+\.tmp$')
strays=$(ls -A "$STATE/philemon" | grep -vc '^ui\.json$\|^ui\.json\.lock$\|^ui\.json\.[0-9]\+\.tmp$')
